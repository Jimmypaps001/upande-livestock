...
class AnimalEvent(Document):
	def before_insert(self):
		...
		if self.event_type == "Pregnancy Diagnosis":
			...
			if self.related_service:
				service = frappe.get_doc("Animal Event", self.related_service)
	...
	def validate(self):
		...
		if self.event_type == "Service":
			...
			if pending_services:
				...
				frappe.throw(f"""<b>⚠️ Cannot record new service!</b><br><br>
                    ...
                    <i>Tip: Go to Animal Events → Find service → Record pregnancy diagnosis</i>""")
		...
		if self.event_type == "Calving":
			...
			service = frappe.get_doc("Animal Event", self.custom_related_pregnancy)
	...
	def on_submit(self):
		...
		# ============================================================
		# Auto-create a Journal Entry when an Animal Event with an activity cost is submitted.
						...
						if not asset_data or not asset_data.asset_category:
							...
							if not cat_account:
								...
								if not expense_account:
									...
									frappe.db.set_value("Animal Event", self.name, "custom_expense_account", expense_account, update_modified=False)
									if cost_center:
										frappe.db.set_value("Animal Event", self.name, "custom_cost_center", cost_center, update_modified=False)
									je = frappe.get_doc({
									...
									je.submit()
									frappe.db.set_value("Animal Event", self.name, "custom_journal_entry", je.name, update_modified=False)
