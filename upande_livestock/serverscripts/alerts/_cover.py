...
"""How many days of concentrate the farm has left, for the cover alert.

NOT A MIXING PLAN ANY MORE. This was `feeding.concentrate_plan`, and it drove
the Concentrate page: days of cover times head counts, rounded up to whole
batches, and that was what the farm was told to mix. It was the wrong shape for
the question — a mixer takes a tonne of ingredients and makes a tonne of meal
whether there are forty cows in the shed or none — so mixing is by the kilo now
and the page no longer asks anything of this.

What survives is the one thing days genuinely answer, and it lives here with
the alert that needs it: COVER IS COUNTED IN DAYS, NOT KILOGRAMS. "412 kg of
calves meal" means nothing without the herd behind it — six weeks for the
calves, two days for the milkers. A concentrate that runs out stops eight herds
at once, on a morning nobody chose, so the alert that says so keeps its
arithmetic.

The original text follows, because the reasoning in it is still the reasoning.
