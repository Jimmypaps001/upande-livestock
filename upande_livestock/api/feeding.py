...
def get_herd_feed_info(herd):
    ...
    available = frappe.db.get_value(
        "Bin", {"item_code": bom.item, "warehouse": store}, "actual_qty"
    )
...
def manufacture_herd_feed(herd):
    ...
    transfer = frappe.get_doc(
        make_stock_entry(wo.name, "Material Transfer for Manufacture", total_qty)
    )
...
def feed_herd(herd, qty, employee=None):
    ...
    se.remarks = "Animal feeding - {0} - {1} - {2} {3}".format(
        herd, item, qty, bom.uom or ""
    )
